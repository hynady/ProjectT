...
public interface OccaRepository extends JpaRepository<Occa, UUID> {
    ...
    // @Query("SELECT new
    // com.ticket.servermono.occacontext.adapters.dtos.OccaResponse(" +
    // "o.id, o.title, o.image, o.date, o.time, o.venue.location, o.price, " +
    // "o.category.id, o.venue.id) " +
    // "FROM Occa o " +
    ...
    // " WHEN :sortBy = 'price' THEN CAST(REPLACE(REPLACE(o.price, '$', ''), ',',
    // '') AS double) " +
    ...
    @Query("SELECT new com.ticket.servermono.occacontext.adapters.dtos.Booking.OccaForBookingResponse(" +
            "o.id, o.title, o.venue.address, o.duration, o.venue.location) " +
            "FROM Occa o " +
            "WHERE o.id = :occaId")
