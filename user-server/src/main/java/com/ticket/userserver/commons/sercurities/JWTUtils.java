package com.ticket.userserver.commons.sercurities;
...
public class JWTUtils {
    ...
    public String createToken(EndUser endUser, boolean rememberMe) {
        long now = (new Date()).getTime();
        Date validity = rememberMe ? new Date(now + TOKEN_VALIDITY_REMEMBER) : new Date(now + TOKEN_VALIDITY);
        Map<String, Object> claims = new HashMap<>();
        claims.put("role", endUser.getRoles());

        return Jwts.builder()
                .setSubject(endUser.getId().toString())
